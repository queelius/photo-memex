...
- ptk view          Manage views
- ptk ai            AI operations
...
def query(
    ...
    view: Optional[str] = typer.Option(None, "--view", "-v", help="Filter by view"),
    field: Optional[List[str]] = typer.Option(None, "--field", help="Field filter (e.g., decade=1980s)"),
    ...
    Examples:
        ...
        ptk q --field decade=1980s         # by annotation value
...
# =============================================================================
# 7. ptk view (subcommand group)
# =============================================================================

view_app = typer.Typer(help="Manage views (materialized annotations)")
...
def view_create(
    ...
    host: Optional[str] = typer.Option(None, "--host", "-H", help="Ollama host:port"),
    force: bool = typer.Option(False, "--force", "-f", help="Overwrite existing"),
) -> None:
    """Create a new view."""
    _require_library()

    from ptk.views import ViewManager, ViewDef, get_loader
    from ptk.views.schema import ComputeSettings
    import os

    loader = get_loader()
    if not loader.get_profile(profile):
        console.print(f"[red]Unknown profile: {profile}[/red]")
        raise typer.Exit(1)

    model = model or os.environ.get("PTK_OLLAMA_MODEL", "llava")
    host = host or os.environ.get("PTK_OLLAMA_HOST", "localhost")

    view_def = ViewDef(
        name=name,
        profile=profile,
        compute=ComputeSettings(model=model, host=host),
    )

    with session_scope() as session:
        manager = ViewManager(session)
        try:
            manager.create_view(view_def, force=force)
...
def view_run(
    name: str = typer.Argument(..., help="View name"),
    limit: Optional[int] = typer.Option(None, "--limit", "-n", help="Max photos to process"),
) -> None:
    """Run view computation (annotate photos)."""
    _require_library()
    ...
    with session_scope() as session:
        ...
        # Get Ollama service
        host = os.environ.get("PTK_OLLAMA_HOST", "localhost")
        port = int(os.environ.get("PTK_OLLAMA_PORT", "11434"))
        model = view.model or os.environ.get("PTK_OLLAMA_MODEL", "llava")

        # Handle host:port format
        if view.model_host:
            if ":" in view.model_host:
                host, port_str = view.model_host.split(":", 1)
                port = int(port_str)
            else:
                host = view.model_host

        service = OllamaVisionService(host=host, port=port, model=model)

        if not service.is_available():
            console.print(f"[red]Ollama not available at {service.base_url}[/red]")
            raise typer.Exit(1)

        console.print(f"[dim]Running {name} with {model}...[/dim]")
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"),
            ...
            def on_progress(current, total, filename):
                ...
                progress.update(task, completed=current, description=f"[cyan]{filename}[/cyan]")

            stats = manager.run_view(name, service, limit=limit, progress_callback=on_progress)

        console.print(f"\n[green]Success: {stats['success']}[/green]")
        if stats['skipped']:
...
def view_delete(
    ...
    with session_scope() as session:
        ...
        if not force:
            if not typer.confirm(f"Delete view '{name}' ({view.photo_count} photos)?"):
                raise typer.Abort()
...
def ai_status() -> None:
    ...
    host = os.environ.get("PTK_OLLAMA_HOST", "localhost")
    port = int(os.environ.get("PTK_OLLAMA_PORT", "11434"))
    model = os.environ.get("PTK_OLLAMA_MODEL", "llava")

    service = OllamaVisionService(host=host, port=port, model=model)

    if service.is_available():
        console.print(f"[green]Ollama available at {service.base_url}[/green]")
...
def ai_describe(
    photo_id: str = typer.Argument(..., help="Photo ID"),
    ...
    with session_scope() as session:
        ...
        console.print(f"[dim]Describing {photo.filename}...[/dim]")
        description = service.describe(Path(photo.original_path))
...
def ai_ask(
    photo_id: str = typer.Argument(..., help="Photo ID"),
    question: str = typer.Argument(..., help="Question to ask about the photo"),
) -> None:
    ...
    with session_scope() as session:
        ...
        console.print(f"[dim]Asking about {photo.filename}...[/dim]")
        response = service.ask(Path(photo.original_path), question)

        console.print(f"\n{response}")


# =============================================================================
# 9. ptk claude (Claude Code skill management)
# =============================================================================

claude_app = typer.Typer(help="Claude Code skill management")
...
def claude_install(
    ...
    """Install ptk skill for Claude Code."""
    from ptk.skills import install

    success, message = install(force=force)

    if success:
        console.print(f"[green]{message}[/green]")
        console.print("\n[dim]Restart Claude Code to activate the skill.[/dim]")
    else:
...
def claude_uninstall() -> None:
    """Remove ptk skill from Claude Code."""
    from ptk.skills import uninstall
...
def claude_status() -> None:
    """Check if ptk skill is installed."""
    from ptk.skills import is_installed, get_skill_dir

    if is_installed():
        console.print(f"[green]Skill is installed[/green]")
        console.print(f"Location: {get_skill_dir()}")
    else:
        console.print(f"[yellow]Skill not installed[/yellow]")
        console.print("Run: ptk claude install")


@claude_app.command("show")
def claude_show() -> None:
    """Display the installed skill content."""
    from ptk.skills import is_installed, get_skill_content

    if not is_installed():
        console.print("[red]Skill not installed.[/red]")
        console.print("Run: ptk claude install")
        raise typer.Exit(1)
    ...
    if content:
        ...
        console.print("[red]Could not read skill content.[/red]")
        raise typer.Exit(1)


# =============================================================================
# 10. ptk verify/relocate/rescan (Path management)
# =============================================================================
